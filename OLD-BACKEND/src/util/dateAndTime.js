 ...
 * appended. If optional second argument "fileName" is true the timestring
 * uses ("-" instead of ":") to make it a valid filename on Windows
 */
export function dateTime(mode, fileName = false, date = new Date()) {
  let timeString;
  let dateString;
  let resultString;
...
/** List matching month-abbreviations to zero-based indexes */
export const months = [
  [0, "Jan"],
  [1, "Feb"],
  [2, "Mar"],
